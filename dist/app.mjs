...
let data,state,island,phrase,epoch=0,recorder=null,stream=null,chunks=[],objectURL=null,clock=null,limit=null,ctx=null,refCache=null,abort=null,reviewLocked=false,pending=false;
...
function resetComparison(){refCache=null;$('comparison-result').replaceChildren(el('p','Depois de gravar, compare o ritmo, a força relativa da voz e o desenho da entonação.','muted'));}
function dispose(){
  epoch++;pending=false;abort?.abort();abort=null;
  ...
  $('record').disabled=false;$('stop').disabled=true;$('record').classList.remove('recording-active');$('record').innerHTML='<span aria-hidden="true">●</span> Gravar';$('timer').textContent='0:00';$('rec-indicator').textContent='LOCAL';
  resetComparison();
}
function phraseList(){
...
function select(id,index,focus=false){
  ...
  $('sentence').textContent=phrase.zh;$('scene').textContent=`${phrase.scene} · ${phrase.register}`;$('phrase-number').textContent=`${index+1} / ${island.phrases.length}`;
  $('cue').textContent=phrase.cue;$('pinyin').textContent=phrase.pinyin;$('translation').textContent=phrase.pt;$('structure').textContent=phrase.note;
  $('pinyin-details').open=state.preferences.pinyin;$('meaning-details').open=state.preferences.meaning;
  reference.src=phrase.audio;reference.playbackRate=1;reference.load();$('ref-status').textContent='Ouça a frase inteira em velocidade normal.';$('rec-status').textContent='Sua gravação fica só nesta frase, neste aparelho.';
...
function stop(){if(recorder&&recorder.state==='recording'){recorder.stop();stream?.getTracks().forEach(t=>t.stop());clearInterval(clock);clearTimeout(limit);$('stop').disabled=true;$('record').classList.remove('recording-active');$('rec-indicator').textContent='LOCAL';$('rec-status').textContent='Preparando sua gravação…';}}
async function startRecording(){
  ...
  try{
    ...
    localRecorder.onstop=async()=>{
      if(ticket!==epoch)return;stream?.getTracks().forEach(t=>t.stop());stream=null;recorder=null;clearInterval(clock);clearTimeout(limit);
    ...
    localRecorder.start(200);$('stop').disabled=false;$('record').classList.add('recording-active');$('rec-indicator').textContent='GRAVANDO';$('rec-status').textContent='Gravando. Toque em Parar quando terminar (limite: 30 segundos).';
...
async function analyze(blob,path,ticket){
  const result=$('comparison-result');result.replaceChildren(el('p','Comparando os sinais neste aparelho…','muted'));
  try{
    ...
    const [recordingBytes,response]=await Promise.all([blob.arrayBuffer(),fetch(path,{signal})]);if(!response.ok)throw Error('Não consegui carregar a referência para comparar. Sua gravação continua disponível para ouvir.');
    const referenceBytes=await response.arrayBuffer();if(ticket!==epoch)return;
    const [a,b]=await Promise.all([currentContext.decodeAudioData(referenceBytes),currentContext.decodeAudioData(recordingBytes)]);if(ticket!==epoch)return;
    const fa=features(a.getChannelData(0),a.sampleRate),fb=features(b.getChannelData(0),b.sampleRate);const score=compare(fa,fb);if(ticket!==epoch)return;
...
function displayScore(s){
  ...
  const row=el('div',undefined,'score-row');row.append(el('strong',s.total===null?'—':`${s.total}%`),el('span',s.total===null?'Sem resultado global: entonação insuficiente para comparar.':'Semelhança acústica estimada'));root.append(row);
...
async function init(){try{const r=await fetch('./content.json');if(!r.ok)throw Error();data=await r.json();if(!data.islands?.length)throw Error();state=readState();select(state.island,state.positions[state.island]||0);$('loading').hidden=true;$('workspace').hidden=false;}catch{$('loading').textContent='Não foi possível abrir o material. Verifique a conexão e recarregue a página.';}}
init();
